/**
 * Guarded wallet session requests: the un-hang seam for every provider
 * request that awaits a human in a wallet (personal_sign /
 * eth_sendTransaction / eth_signTypedData_v4). Moved from
 * apps/mobile/wallet/reown/request-guard.ts 2026-08-15; web shares it —
 * AppKit's browser providers ride the same relay-or-extension promise that
 * can silently never settle.
 *
 * WC responses ride a cloud relay websocket. When the app is backgrounded
 * while the wallet is up, that socket can drop and the wallet's response is
 * lost — the `provider.request` promise then NEVER settles, which used to
 * freeze the signing modal until the app was killed. The guard races every
 * request against a timeout and a user-driven abort (the modal's Cancel
 * button), so silence becomes a typed `WalletError` instead of a hang.
 *
 ...
 * cleanup, so BOTH exits disconnect best-effort; the teardown is per-client
 * transport, so `disconnect` is INJECTED — reconnect stays one tap via each
 * client's connect-on-demand guard.
 *
 ...
 * dead button on paths that don't route through here (e.g. Solana/MWA).
 */
...
/** Whether an abortable wallet request is currently awaiting the wallet. */
...
/**
 * Race a wallet request against the timeout + abort exits. Returns the
 * wallet's result when it answers in time; throws `WalletError('timeout')` /
 ...
 * transactions).
 */
export async function guardWalletRequest<T>(
  pending: Promise<T>,
  opts: { disconnect: () => Promise<void>; timeoutMs?: number },
): Promise<T> {
