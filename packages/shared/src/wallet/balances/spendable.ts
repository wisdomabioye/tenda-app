 ...
 * the largest balance held by any of `owners` (moved from
 * apps/mobile/wallet/balances/spendable.ts 2026-08-15).
 *
...
export async function readSpendableBalance(
  ...
  const readings = await Promise.all(
    owners.map((owner) => readAssetBalance(owner, chain, assetId, readers).catch(() => null)),
  )
