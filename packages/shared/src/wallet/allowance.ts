 ...
 * screen (read / set / revoke, independent of any escrow). Moved from
 * apps/mobile/wallet/allowance 2026-08-15.
 *
 ...
 * wallet session, which is per-client transport — so it is INJECTED
 * (`sendTx`), keeping this module zero-dependency: mobile passes its
 * WalletConnect sender, web its AppKit provider sender. Keys never leave
 * the wallet either way.
 */
import { evmPublicRpcUrl } from '../chains'
...
/** Receipt poll bounds: wallets can sit backgrounded for a while. */
export const RECEIPT_POLL_INTERVAL_MS = 2_000
export const RECEIPT_POLL_TIMEOUT_MS = 120_000

/**
 * The per-client transport seam: broadcast a prepared EVM call through the
 * connected wallet session and return the tx hash. `value` is a decimal wei
 * string (the transport hex-encodes it).
 */
export type SendEvmTx = (input: {
  from: string
  to: string
  data: string
  value: string
