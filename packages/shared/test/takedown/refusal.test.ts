...
import { isTakedownRefusal } from '../../src/takedown/refusal'
import { ApiClientError } from '../../src/api/client-error'

test('the takedown code is a refusal, whatever status carries it', () => {
  assert.strictEqual(
    isTakedownRefusal(new ApiClientError(409, 'Conflict', 'gone', 'ESCROW_TAKEN_DOWN')),
    true,
  )
})
...
test('other statuses and code-less envelopes are not takedown refusals', () => {
  ...
  for (const [label, thrown] of cases) {
    assert.strictEqual(isTakedownRefusal(thrown), false, label)
  }
})

test('non-envelope throws are never takedown refusals', () => {
  const cases: [string, unknown][] = [
    ...
    ['null', null],
    ['undefined', undefined],
    // The shape that would slip through a duck-typed check. Only a real
    // envelope from the server can prove the listing was pulled.
