...
/**
 * The Button props one width implies. Shared by both renderers so the two
 * cannot drift — `fullWidth` and `flex: 1` fight each other (width 100% vs
 * flex), and getting that wrong in one file only is invisible until a device.
 */
