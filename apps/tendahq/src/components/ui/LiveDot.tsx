...
export function LiveDot({ size = 8, pulseMs = 1600 }: Props) {
  return (
    ...
    >
      <span
        className="absolute inset-0 rounded-full bg-[var(--success)] opacity-60 motion-safe:animate-[live-ping_var(--live-pulse-ms)_ease-out_infinite]"
        style={{ ['--live-pulse-ms' as string]: `${pulseMs}ms` }}
      />
      <span
        className="relative rounded-full bg-[var(--success)]"
        style={{ width: size, height: size }}
      />
    </span>
  )
