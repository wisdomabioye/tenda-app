 ...
 * the connect modal. Isolated from reown-networks.test.ts because proving it
 * needs a mocked manifest and a fresh module instance.
 */
...
describe('module-init fail-loud contract', () => {
  test('a solana manifest chain with no preset fails the module import itself', async () => {
    vi.resetModules()
    ...
    await expect(import('@/providers/reown/networks')).rejects.toThrow(
