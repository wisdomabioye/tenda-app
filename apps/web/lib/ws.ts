/**
 * Singleton WebSocket client — verbatim port of apps/mobile/lib/ws.ts
 * (stage-2-listeners.md § Mobile; stage-5 doc: "lib/ws.ts needs no
 * changes"). The only web substitutions are the two import seams:
 * getJwtToken comes from lib/storage (localStorage) instead of
 * secure-store, getEnv from lib/config/env.
 *
...
async function open(): Promise<void> {
  ...
  try {
    ...
    // Storage can be temporarily unavailable (private-mode quirks, a
    // just-cleared origin). Treat that like any other disconnected state
    // and retry.
    scheduleReconnect()
    return
  } finally {
...
export const ws = {
  /** Idempotent, call on sign-in / tab foreground. */
  connect(): void {
    state.enabled = true
    void open()
  },
