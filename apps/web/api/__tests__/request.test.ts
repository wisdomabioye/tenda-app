 ...
 *
 * Ported from apps/mobile/api/__tests__/request.test.ts (jest → vitest); the
 * only web difference is the storage module the JWT comes from.
 */
...
function mockFetch(status = 200, body: unknown = { ok: true }): Mock {
...
function sentHeaders(fetchMock: Mock): Record<string, string> {
  return (fetchMock.mock.calls[0] as [string, RequestInit])[1].headers as Record<string, string>
}
...
describe('request, timeout budget', () => {
  it('maps an expired request deadline to a typed API timeout', async () => {
    vi.useFakeTimers()
    getJwt.mockResolvedValue(null)
    global.fetch = vi.fn((_url: RequestInfo | URL, init?: RequestInit) => new Promise((_resolve, reject) => {
...
describe('request, query serialisation', () => {
  function sentUrl(fetchMock: Mock): string {
