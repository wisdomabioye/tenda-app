/**
 * Web port of apps/mobile/hooks/useDisputeThread.ts, verbatim: loads the
 * CO7 dispute thread for an escrow, then tails it with the project-
 * standard recursive-setTimeout poll (backs off after 3 empty polls, same
 * cadence as chat's useMessagePolling). Only SERVER batches advance the
 * poll cursor — advancing it from our own send() would skip a
...
export interface DisputeThreadState {
  ...
  /**
   * Append a message (optionally with an attachment). Reports WHY a send
   * was refused: only `failed` is worth retrying.
   */
