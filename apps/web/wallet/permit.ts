/**
 * Client leg of the EIP-2612 permit flow — web port of mobile's
 * wallet/permit.ts. ONE entry point the create and dispute flows call before
 * hitting their escrow endpoint:
 *
...
import { signEvmTypedData } from './send/evm'
import { ensureSessionOn } from './send/session'
import { resolveEvmFrom } from './dispatch'
...
export async function buildPermitFor(args: {
  ...
  // session here too (idempotent — a no-op when dispatch already connected
  // one), otherwise the permit signature would dead-end with no provider.
