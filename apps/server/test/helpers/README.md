...
| `x402-recording.ts` | `sameShape()`, `VOLATILE`, `writeRecording()` — comparing the PUBLISHED x402 recording (`src/agent-api/recorded-exchange.ts`, inlined into the slim document as #109's examples) against a fresh capture, and re-writing it via `pnpm record:x402`. Shape and stable values are both compared; the fields that differ every run are named one by one in `VOLATILE` rather than skipped by a loose match |
